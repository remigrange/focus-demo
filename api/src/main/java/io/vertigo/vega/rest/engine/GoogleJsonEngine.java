    ...
    private Gson createGson() {
        ...
        .registerTypeAdapter(ComponentInfo.class, new JsonSerializer<ComponentInfo>() {
            ...
            public JsonElement serialize(final FacetedQueryResult src, final Type typeOfSrc,
                ...
                final JsonObject jsonObject = new JsonObject();
                if (src.getClusters().size() > 0) {
                    final Map<FacetValue, DtList<DtObject>> clusterList = src.getClusters();
                    final Map<String, DtList<DtObject>> mapsClusters = new HashMap();
                    for (final Entry<FacetValue, DtList<DtObject>> entry : clusterList.entrySet()) {
                        final DtList<DtObject> list = entry.getValue();
                        mapsClusters.put(entry.getKey().getLabel().getDisplay(), list);
                    }
                    final JsonObject jsonData = (JsonObject) context.serialize(mapsClusters);
